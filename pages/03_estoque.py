...
st.title("📦 Estoque de Produto de Limpeza")
...
if estoque <= 0:
    ...
    st.warning(f"⚠️ Estoque baixo: {estoque} unidade(s). Considere reabastecer.")
else:
    st.success(f"✅ Estoque OK: {estoque} unidade(s)")

progresso = min(estoque / 20, 1.0)  # 20 unidades = 100%
...
with st.form("form_compra"):
    st.markdown("""
    **Referência:** 5 litros = 10 unidades de 500ml = R$ 90,00
    """)
    qtd_litros = st.number_input(
        "Quantidade (litros)", min_value=1.0, max_value=100.0, value=5.0, step=0.5,
    )
    unidades = int(qtd_litros * 2)
    custo = (qtd_litros / 5) * 90.0

    st.markdown(f"**{unidades} unidades** de 500ml | Custo: **R$ {custo:.2f}**")

    if st.form_submit_button("Registrar Compra", type="primary", use_container_width=True):
        db.registrar_compra_produto(qtd_litros)
        st.success(f"Compra registrada! +{unidades} unidades. Despesa de R$ {custo:.2f} lançada automaticamente.")
        st.rerun()
...
if movs:
    for m in movs:
        ...
        st.text(f"{emoji} {m['data'][:16]} | {sinal}{m['quantidade']} unid.{custo_str} | {m['observacoes'] or ''}")
else:
